...
$pdf->Cell(35, 2, $general['username'], 0, 1, 'R');
$pdf->Cell(70, 6, '-----------------------------------------------------------------------------', 0, 1, 'C');
$pdf->SetFont('helvetica', '', 8);
...
foreach ($detail as $d) {
    ...
    $pdf->Cell(20, 2, 'Rp. ' . number_format($d['subtotal'], '0', '.', '.'), 0, 1, 'R');
...
foreach ($servis as $val) {
    $pdf->Cell(49, 2, $val['nama_servis'], 0, 0, 'L');
    $pdf->Cell(4, 2, 'x' . $val['qty_jual'], 0, 0, 'L');
    $pdf->Cell(20, 2, $val['subtotal'], 0, 1, 'R');
}

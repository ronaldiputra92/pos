// Konfigurasi DataTables untuk Customer dengan deskripsi paket yang lengkap
$(document).ready(function() {
    // Hancurkan DataTable yang sudah ada jika ada
    if ($.fn.DataTable.isDataTable('#datacustomer')) {
    ...
    var tableCustomer = $('#datacustomer').DataTable({
        "bProcessing": false,
        ajax: {
            url: base_url + 'customer/LoadData',
            type: 'POST',
            dataSrc: 'aaData'
        },
        "bJQueryUI": true,
        ...
        "aoColumns": [{
            ...
            },
            {
                "mDataProp": function (data, type, val) {
                    // Format jenis_cs dengan deskripsi
                    var paket = '';
                    switch(data.jenis_cs) {
                        case 'Studio 1':
                            paket = 'Studio 1 (Self Photo)';
                            break;
                        case 'Studio 2':
                            paket = 'Studio 2 (Self Photo)';
                            break;
                        case 'Studio 3':
                            paket = 'Studio 3 (Wide Photobox)';
                            break;
                        case 'Studio 4':
                            paket = 'Studio 4 (Photo Elevator)';
                            break;
                        default:
                            paket = data.jenis_cs;
                            break;
